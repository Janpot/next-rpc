...
function visitPage({ types: t }, path, { isServer, pagesDir, dev, apiDir }) {
  ...
  const wrapGetPropsIdentifier = path.scope.generateUidIdentifier(
    'wrapGetProps'
  );

  path.node.body.unshift(
    t.importDeclaration(
      [t.importSpecifier(wrapGetPropsIdentifier, t.identifier('wrapGetProps'))],
      t.stringLiteral('next-rpc/context')
    )
  );
    ...
    ExportNamedDeclaration(path) {
      ...
      if (
        ...
        (t.isIdentifier(declaration.id, { name: 'getServerSideProps' }) ||
          t.isIdentifier(declaration.id, { name: 'getStaticProps' }))
      ) {
        /** @type {import('@babel/core').types.Expression} */
        ...
        path.node.declaration = t.variableDeclaration('const', [
          t.variableDeclarator(
            ...
            annotateAsPure(
              t,
              t.callExpression(wrapGetPropsIdentifier, [exportAsExpression])
            )
          ),
        ]);
      }
